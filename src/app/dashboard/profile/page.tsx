...
import { User, Mail, Shield, Key, Trash2, Camera, CheckCircle2, AlertTriangle } from "lucide-react";
      ...
      <div className="relative mb-16 rounded-2xl bg-surface/30 border border-border overflow-hidden">
        ...
        <div className="absolute -bottom-8 left-8 flex items-end gap-5">
          <div className="relative">
            {user?.image ? (
              <img src={user.image} alt="" className="h-24 w-24 rounded-full border-4 border-bg object-cover" />
            ) : (
          ...
          <div className="mb-2">
            <h1 className="text-2xl font-bold text-text">{user?.name || "Grob User"}</h1>
            <p className="text-sm font-medium text-muted">@{user?.email?.split('@')[0] || "user"}</p>
          </div>
        </div>
      </div>
          ...
          <section id="general" className="space-y-4">
            ...
            <div className="rounded-xl border border-border bg-surface/20 overflow-hidden">
              ...
              <div className="flex items-center justify-between border-t border-border bg-surface/30 px-5 py-3">
                ...
                <button className="rounded-lg bg-text px-4 py-1.5 text-xs font-semibold text-bg transition-transform hover:scale-[1.02] active:scale-[0.98]">
                  Save Changes
                </button>
              </div>
            </div>
                ...
                <div className="flex items-center justify-between p-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted">Set a permanent password to login to your account.</p>
                    </div>
                  </div>
                  <button className="rounded-lg border border-border bg-transparent px-3 py-1.5 text-xs font-medium text-text transition-colors hover:bg-white/5">
                    Update
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between p-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted">Connect GitHub to login instantly.</p>
                    </div>
                  </div>
                  <button className="rounded-lg border border-border bg-transparent px-3 py-1.5 text-xs font-medium text-text transition-colors hover:bg-white/5">
                    Connect
          ...
          <section id="danger" className="space-y-4 pt-4">
            ...
            <div className="rounded-xl border border-red-500/20 bg-red-500/5">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5">
                ...
                <button className="flex items-center gap-2 rounded-lg bg-red-500/10 border border-red-500/20 px-4 py-2 text-sm font-medium text-red-500 transition-colors hover:bg-red-500 hover:text-white self-start md:self-auto shrink-0">
